...
* __Server side__ - In [fsf-server/conf/conf.py](fsf-server/conf/conf.py) 
 * Make sure you are pointing to your master yara signature file using the full path. See [fsf-server/yara/rules.yara](fsf-server/yara/rules.yara)
 * Set the logging directory; make sure it exists and ensure you have permissions to write to it
 * In [fsf-server/](fsf-server), start up the server using `./main.py start` and it will daemonize 
* __On client__ - In [fsf-client/conf/conf.py](fsf-client/conf/conf.py)
